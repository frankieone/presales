        ...
        <div className="px-6 py-4 overflow-y-auto flex-1">
          {loading ? (
            ...
            <div className="text-sm text-wise-gray-500 py-4">
              ...
              {/* Trust linking */}
              <div className="mt-4 pt-4 border-t border-wise-gray-100">
                {hasTrust ? (
                  isTrustLinked ? (
                    <div className="flex items-center justify-between rounded-lg border-2 border-yellow-300 bg-yellow-50 px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <svg className="w-4 h-4 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className="text-xs font-medium text-yellow-800">Trust linked to this organisation</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => onUnlinkTrust?.()}
                    ...
                    <button
                      type="button"
                      onClick={() => onLinkTrust?.(entityId)}
                      className="w-full flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-yellow-300 bg-yellow-50/50 px-3 py-2.5 hover:bg-yellow-100 hover:border-yellow-400 transition-colors"
                    >
                      <svg className="w-4 h-4 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                      </svg>
                      <span className="text-xs font-medium text-yellow-800">Link Trust to this Organisation</span>
                    </button>
                  )
                ) : (
                  <div className="rounded-lg border-2 border-dashed border-yellow-300 bg-yellow-50/50 px-3 py-3">
                    <div className="flex items-center gap-2 mb-1.5">
                      <svg className="w-4 h-4 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                      <span className="text-xs font-semibold text-yellow-800">Trust Document Required</span>
                    </div>
                    <p className="text-[11px] text-yellow-700">
                      If this organisation is a trust, upload the trust deed document below in the Supporting Documents section and confirm it to link it here.
                    </p>
                  </div>
                )}
              </div>
