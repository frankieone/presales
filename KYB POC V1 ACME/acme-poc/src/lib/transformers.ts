...
export function extractAustralianIndividuals(
  ...
  for (const oh of officeholders) {
    ...
    const role = mapAustralianRole(oh.role);
    const addr = oh.addresses?.[0];
    // ABR only provides postcode and state — not full address
    // DOB is only available for directors (registered with ASIC)
    ...
    individuals.push({
      ...
      middleName,
      familyName,
      dateOfBirth: isDirector ? (oh.dateOfBirth || undefined) : undefined,
      kycEntityId: oh.entityId || undefined,
      ownershipEntityId: oh.entityId || undefined,
      address: addr
        ? {
            state: addr.state,
  ...
  for (const ubo of ubos) {
    ...
    // UBOs are shareholders — DOB is not available from the registry
    // ABR only provides postcode and state
    individuals.push({
      id: uuidv4(),
      givenName,
      middleName,
      ...
      ownershipEntityId: ubo.entityId || undefined,
      address: addr
        ? {
            state: addr.state,
            postalCode: addr.postalCode,
...
export function deduplicateIndividuals(individuals: Individual[]): Individual[] {
  ...
  for (const ind of individuals) {
    ...
    if (existing) {
      ...
      if (!existing.ownershipEntityId && ind.ownershipEntityId)
        existing.ownershipEntityId = ind.ownershipEntityId;
    } else {
      map.set(key, { ...ind });
    }
  }
